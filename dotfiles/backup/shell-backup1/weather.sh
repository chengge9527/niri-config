...
read -r temp feels_like humidity icon_cod  desc < <(
    jq -r '[
        .main.temp,
        .main.feelsLike,
        .main.humidity,
...
#     precip_info+=$'\n'"降雪量(1h): ${snow_1h} mm"
# fi
    local icon_result=$(get_weather_icon "$icon_cod")
    local icon=$(echo "$icon_result" | cut -d'|' -f1)
    local class=$(echo "$icon_result" | cut -d'|' -f2)
# if [[ "$snow_1h" != "0" ]]; then
#     precip_info+=$'\n'"降雪量(1h): ${snow_1h} mm"
# fi
